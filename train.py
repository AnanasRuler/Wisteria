...
def print_model_info(model, config=None):
    ...
    # 打印模型配置信息
    if config and hasattr(config, 'model') and hasattr(config.model, 'config'):
        model_config = config.model.config
        print(f"Model Type: {config.model.get('_name_', 'Unknown')}")
        print(f"Model Configuration:")
        
        # 打印关键配置参数
        key_params = [
            'd_model', 'n_modules', 'layers_per_module', 'conv_layers_per_module',
            'attn_layer_in_module', 'n_layer', 'vocab_size', 'bidirectional',
            'bidirectional_strategy', 'dilation_base'
        ]
        ...
        # 打印层索引信息
        if hasattr(model_config, 'attn_layer_idx'):
            print(f"  Attention Layer Indices: {model_config.attn_layer_idx}")
        if hasattr(model_config, 'MSC_layer_idx'):
            print(f"  MSC Layer Indices: {model_config.MSC_layer_idx}")
    ...
    if hasattr(model, 'model'):
        ...
        print(f"Core Model Parameters: {model_params:,}")
    ...
    try:
        ...
        # 检查传统的层结构
            ...
            print(f"\nModel Layer Structure (Legacy):")
            if hasattr(model.model.layers, '__len__'):
                print(f"  Number of layers: {len(model.model.layers)}")
                for i, layer in enumerate(model.model.layers):
                    if hasattr(layer, 'parameters'):
                        layer_params = sum(p.numel() for p in layer.parameters())
                        layer_type = type(layer).__name__
                        print(f"  Layer {i} ({layer_type}): {layer_params:,} parameters")
    except Exception as e:
        print(f"  Could not analyze module structure: {e}")
    ...
    # 打印完整的模型结构（限制长度）
    model_str = str(model)
    if len(model_str) > 15000:  # 如果模型结构太长，截断显示
        print(model_str[:7500])
        print("\n... [MODEL STRUCTURE TRUNCATED] ...\n")
        print(model_str[-5000:])
    else:
        print(model)
...
class SequenceLightningModule(pl.LightningModule):
    def __init__(self, config):
        ...
        self._model_printed = False  # 添加标志，确保只打印一次模型信息
        ...
        (
            self.metrics,
            self.train_torchmetrics,
            self.val_torchmetrics,
            self.test_torchmetrics,
        ) = None, None, None, None
        self.setup()
    ...
    def setup(self, stage=None):
        ...
        if (
            self.hparams.model.get("config", None) is not None
            and self.hparams.model.config.get("_target_", None) is not None
        ):
        ...
        # 打印模型信息（只打印一次）
        if not self._model_printed:
            print_model_info(self, self.hparams)
    ...
    def _process_state(self, batch, batch_idx, training=True):
        ...
        # Pass through memory chunks
            ...
            with (
                torch.no_grad()
            ):  # should be unnecessary because individual modules should handle this
    ...
    def _shared_step(self, batch, batch_idx, prefix="train"):
        ...
        x, y, w = self._shared_forward(batch)
    ...
    def training_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        if MHA.LOAD_BALANCING_LOSSES:
            load_balancing_loss = sum(MHA.LOAD_BALANCING_LOSSES)
            MHA.LOAD_BALANCING_LOSSES.clear()  # 清空列表，准备下一轮

        # 将负载均衡损失加到主损失上，使用权重系数调节
        loss = loss + 0.01 * load_balancing_loss
    ...
    def configure_optimizers(self):
        ...
        print(
            "Hyperparameter groups:", hps
        )  # TODO: log.info throws error because hps is list of dicts
        for hp in hps:
...
def train(config):
    ...
    if config.train.test:
        ...
        ):  # First, load the best validation model
